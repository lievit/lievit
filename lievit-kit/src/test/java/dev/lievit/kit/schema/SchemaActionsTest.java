...
class SchemaActionsTest {
    ...
    void actions_render_inline_buttons_and_run_over_the_live_state() {
        ...
        assertThat(row.isDehydrated(dev.lievit.kit.support.EvaluationContext.of(null))).isFalse();

        SchemaState s = state(new java.util.LinkedHashMap<>(Map.of("title", "Hello World")));
        MutableEvaluationContext mctx =
                MutableEvaluationContext.over(null, null, dev.lievit.kit.support.EvaluationContext.Operation.CREATE, s);
        generate.run(mctx);
    ...
    void a_disabled_schema_action_is_a_no_op() {
        ...
        action.run(dev.lievit.kit.support.EvaluationContext.of(null));
