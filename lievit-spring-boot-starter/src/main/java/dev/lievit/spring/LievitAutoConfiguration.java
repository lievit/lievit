...
import dev.lievit.compiler.DeterministicKeys;
import dev.lievit.compiler.DirectiveValidator;
import dev.lievit.component.BeanValidationFieldValidator;
import dev.lievit.component.FieldValidator;
import dev.lievit.component.IsolateListener;
import dev.lievit.component.LifecycleBus;
import dev.lievit.component.LifecycleHooksListener;
import dev.lievit.component.LifecyclePhase;
import dev.lievit.component.LocaleListener;
import dev.lievit.component.MagicActionListener;
import dev.lievit.component.NoOpFieldValidator;
import dev.lievit.component.RedirectListener;
import dev.lievit.component.RenderlessListener;
import dev.lievit.component.SessionListener;
import dev.lievit.component.TransitionListener;
import dev.lievit.component.WireDispatcher;
import dev.lievit.dsl.DslOrEngineTemplateAdapter;
import dev.lievit.dsl.DslTemplateAdapter;
import dev.lievit.jte.JteTemplateAdapter;
import dev.lievit.render.TemplateAdapter;
import dev.lievit.spring.native_.LievitRuntimeHints;
import dev.lievit.wire.ChecksumFailureLimiter;
import dev.lievit.wire.ComponentId;
import dev.lievit.wire.PayloadGuard;
import dev.lievit.wire.SigningKeys;
import dev.lievit.wire.SnapshotCodec;
...
public class LievitAutoConfiguration {
    ...
    public WireDispatcher lievitWireDispatcher(
            ...
            ObjectProvider<dev.lievit.component.ActionAuthorizer> actionAuthorizer) {
        ...
        dev.lievit.component.ActionAuthorizer authorizer =
                actionAuthorizer.getIfAvailable(dev.lievit.component.ActionAuthorizer::permitAll);
    ...
    @Bean
    @ConditionalOnMissingBean
    public dev.lievit.upload.TempFileSigner lievitTempFileSigner(LievitProperties properties) {
        ...
        return new dev.lievit.upload.TempFileSigner(decode(key), properties.getUploadPreviewTtl());
    }
    ...
    @Bean
    @ConditionalOnMissingBean
    public dev.lievit.spring.upload.TempFileStorage lievitUploadStorage(LievitProperties properties) {
        ...
        return new dev.lievit.spring.upload.TempFileStorage(root);
    }
    ...
    @Bean
    @ConditionalOnMissingBean
    public dev.lievit.spring.upload.LievitUploadController lievitUploadController(
            dev.lievit.spring.upload.TempFileStorage storage,
            dev.lievit.upload.TempFileSigner signer,
            LievitProperties properties) {
        dev.lievit.upload.UploadConstraints constraints =
                new dev.lievit.upload.UploadConstraints(properties.getUploadMaxBytes(), java.util.Set.of());
        return new dev.lievit.spring.upload.LievitUploadController(storage, signer, constraints);
    }

    /**
     * The default local-filesystem {@link dev.lievit.upload.FileStore} (issue #189): moves a validated
     * {@link dev.lievit.upload.TemporaryUploadedFile} from the temp area to a permanent root. Conditional
     ...
     */
    @Bean
    @ConditionalOnMissingBean(dev.lievit.upload.FileStore.class)
    public dev.lievit.upload.FileStore lievitFileStore(
            dev.lievit.spring.upload.TempFileStorage storage,
            dev.lievit.upload.TempFileSigner signer,
            LievitProperties properties) {
        ...
        return new dev.lievit.spring.upload.LocalFileStore(storage, signer, root);
    }
    ...
    @ConditionalOnMissingBean
    public dev.lievit.spring.upload.TempUploadCleanup lievitUploadCleanup(
            dev.lievit.spring.upload.TempFileStorage storage, LievitProperties properties) {
        dev.lievit.spring.upload.TempUploadCleanup cleanup =
                new dev.lievit.spring.upload.TempUploadCleanup(
    ...
    @org.springframework.boot.autoconfigure.condition.ConditionalOnProperty(
            ...
            name = "enabled",
            havingValue = "true")
    public dev.lievit.spring.broadcast.SseBroadcastChannel lievitBroadcastChannel(
            tools.jackson.databind.ObjectMapper json, LievitProperties properties) {
        return new dev.lievit.spring.broadcast.SseBroadcastChannel(
    ...
    @org.springframework.boot.autoconfigure.condition.ConditionalOnProperty(
            ...
            name = "enabled",
            havingValue = "true")
    public dev.lievit.spring.broadcast.BroadcastController lievitBroadcastController(
            dev.lievit.spring.broadcast.SseBroadcastChannel channel) {
        return new dev.lievit.spring.broadcast.BroadcastController(channel);
    }
    ...
    /**
     * Binds the per-request {@link dev.lievit.component.LievitResponse} sink around every request and
    ...
    @Bean
    @ConditionalOnMissingBean
    public dev.lievit.compiler.ComponentCompiler lievitComponentCompiler() {
        return new dev.lievit.compiler.ComponentCompiler();
    }
     ...
     * {@code lievit.assets.classpath-dir}; {@link dev.lievit.compiler.AssetManifest#EMPTY} when no
    ...
    @Bean
    @ConditionalOnMissingBean
    public dev.lievit.compiler.AssetManifest lievitAssetManifest(
    ...
    public ComponentAssetEmitter lievitComponentAssetEmitter(
            dev.lievit.compiler.ComponentCompiler compiler,
            dev.lievit.compiler.AssetManifest manifest) {
