...
public record WireCallRequest(
    ...
    /**
     * @return the inbound events as core {@link dev.lievit.component.InboundEvent}s, never null (an
     *     absent {@code _events} is an empty list)
     */
    public List<dev.lievit.component.InboundEvent> inboundEvents() {
        if (events == null) {
            return List.of();
        }
        return events.stream()
                .map(e -> new dev.lievit.component.InboundEvent(e.name(), e.detail()))
                .toList();
    }
}
