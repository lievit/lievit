...
import dev.lievit.component.WireCall;
...
import dev.lievit.wire.Snapshot;
 ...
 *
 * <p>Real-time validation: when the dispatcher's {@link dev.lievit.component.FieldValidator}
...
public final class LievitWireService {
    ...
    public WireCallResult mount(String className) {
        ...
        dev.lievit.component.LazyComponent lazy =
                dev.lievit.component.LazyComponent.of(instance.getClass());
        if (lazy.isLazy()) {
        ...
        dev.lievit.component.ComponentStack stack = new dev.lievit.component.ComponentStack();
        dev.lievit.component.ComponentStack.bind(stack);
        try {
            stack.push(registry.nameOf(className), cid);
            html = childRenderer.substitute(html, mounted.children());
        } finally {
            dev.lievit.component.ComponentStack.clear();
        }
     ...
     *
     * <p>When the {@link dev.lievit.component.FieldValidator} finds errors, they are
    ...
    public WireCallResult call(
            ...
            Map<String, Object> updates,
            List<String> calls,
            List<dev.lievit.component.InboundEvent> inboundEvents,
            String client) {
    ...
    WireCallOutcome runCall(
        ...
        if (call.renderSkipped()) {
            ...
            if (!islands.isEmpty()) {
                String targeted = dev.lievit.compiler.IslandFragments.extractTargeted(html, islands);
    ...
    private WireEffects.@Nullable Assets deriveAssets(ComponentMetadata metadata, WireCall call) {
        ...
        for (dev.lievit.component.ChildComponent child : call.children()) {
            try {
                accumulate(
     ...
     */
    public String encodeStreamChunk(dev.lievit.component.StreamChunk chunk) {
        try {
