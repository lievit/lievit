...
### DOM patching: a bespoke morph (ADR-0015)
...
- ADR-0001 originally named **Idiomorph**; ADR-0015 amends that to a bespoke implementation of the
...
### The client runtime bundle (ADR-0015)
...
hooks (`beforeCall`/`afterCall`/`onError`/`onModelChange`/`onComponentInit`). See ADR-0015.
...
- ADR-0015 — the client runtime bundle (wire glue, bespoke morph, directive + lifecycle extension points).
