...
At the **completeness bar (full only)**, lievit covers roughly **29%** of the audited Filament + shadcn
surface: **42 of 147** features are `full`, **94 (64%)** are `partial`, **11 (7%)** are `missing`.
Read the other way: lievit ships *something* for **87%** of the surface (`full` + `partial`), and the
shadcn side has materially closed: **27 of 59** shadcn features are now `full`. The remaining pattern
is still **broad, thin** on the Filament side: nearly every field/column/entry/action has a
counterpart, but the deep configuration surface (closures/utility-injection, async/relationship
machinery, storage+editor depth) is where it stops.
The `full` items now span the **self-contained shadcn presentational primitives** (Alert,
Aspect Ratio, Empty, Label, Progress, Separator, Skeleton, Spinner, Switch, Table) **plus the
overlay family and the form/input stack** (Dialog, Sheet, Alert Dialog, Form, Field, Checkbox,
Input, Textarea, Native Select, Radio Group, Slider, Input OTP, Badge, Card, Breadcrumb,
Pagination, Scroll Area) plus exactly **two Filament features** (the deliberately-minimal `Hidden`
field and the three-state `TrashedFilter`).
**Headline gaps**: (1) on the **shadcn side**, the overlay focus-trap and `Form`/validation gaps are
now closed; what remains absent is the heavy interactive set (`Calendar` DayPicker, `Carousel`,
`Resizable`, `Sonner`, `Menubar`, `Typography`); (2) on the **Filament side**, the whole **dynamic-closure /
...
### Area: Filament Forms (fields) — 3 full / 20 partial / 1 missing
...
| Toggle | **full** | `schema/Toggle.java` | — (boolean Toggle distinct from ToggleButtons: on/off icon+color, inline, accepted()/declined()) |
...
| Textarea | **full** | `schema/Textarea.java` | — (schema-engine Textarea, distinct from legacy TextareaField: rows/autosize/readOnly/minLength/maxLength/trim, trim wired through dehydrateStateUsing) |
...
### Area: Filament Tables — 5 full / 23 partial / 3 missing
...
| Custom columns | **full** | `kit/ViewColumn.java` | — (`ViewColumn::make()` arbitrary view + state injection) |
...
| Column groups | **full** | `kit/ColumnGroup.java` | — (`ColumnGroup::make` header grouping) |
...
| TernaryFilter | **full** | `kit/TernaryFilter.java` | — (per-state labels, query closures, nullable(), attribute()) |
...
| Filter layout & persistence | **full** | `Filter.java`, `kit/Table.java` | — (filtersLayout, defer/apply, persistFiltersInSession, default filters + indicators) |
...
### Area: Filament Infolists + Actions — 7 full / 6 partial / 0 missing
...
| Section (layout) | **full** | `infolist/Section.java` | — (infolist Section schema: heading/description/icon/aside/collapsible/actions) |
| Tabs (layout) | **full** | `infolist/Tabs.java` | — (infolist Tab children, activeTab, persistTab, contained, vertical) |
| Grid/Fieldset/Split (layout) | **full** | `infolist/Grid.java`, `infolist/Fieldset.java`, `infolist/Split.java` | — (per-breakpoint columns, columnSpan/Start, Fieldset, Split for entries) |
...
| Action modals | **full** | `ConfirmationModal.java`, `ModalConfig.java`, `SlideOver.java` | — (modalIconColor/alignment, modalContent, extra footer actions, modalCloseButton, sticky header/footer, width-as-enum) |
| Action forms | **full** | `FormAction.java` | — (disabledForm() read-only, arguments() context channel) |
| Action wizards | **full** | `FormAction.java` | — (multistep wizard inside an action modal) |
| Action lifecycle | **full** | `AdminAction.run()`, `AdminActionResult` | — (before()/after()/halt()/cancel(), per-action success/failure notification builders, mountUsing, replaceMountedAction, keyBindings, arguments) |
...
| Panels: authentication | partial | `AuthPage.java` (LoginPage/RegisterPage/PasswordResetPage/EmailVerificationPage), `Panel` handler overloads, `PanelAccessGate.java`, `AccountWidget.java` | MFA, email-change verify, avatar provider, full profile (backing pages now FULL: four real AuthPage models render fields + react to submits through the lievit runtime, AuthPageRenderIT, Panel handler-accepting overloads back each flag) |
...
| Resources: soft deletes/authz/bulk | partial | `PolicyAdminAuthorizer.java`, `AdminOperation.java`, `SoftDeleteRepository`, `ImportAction.java` | sensitive-attribute protection, import/export as AdminAction subclasses (per-verb policy map now FULL: AdminOperation carries the Laravel-Policy ability set incl. RESTORE/FORCE_DELETE/REORDER, PolicyAdminAuthorizer is a default-deny record-aware per-verb rule map) |
...
### Area: shadcn/ui — 27 full / 25 partial / 7 missing
...
| Alert Dialog | **full** | `wire/alert-dialog` | — (focus-trap + Escape + focus-on-cancel now shipped) |
...
| Badge | **full** | `jte/badge` | — (asChild polymorphic render + shadcn variant taxonomy now shipped) |
| Breadcrumb | **full** | `jte/breadcrumb` | — (ellipsis/collapsed dropdown + responsive collapse + asChild now shipped) |
...
| Card | **full** | `jte/card` | — (Title/Description/Content/Footer/Action distinct slots now shipped) |
...
| Checkbox | **full** | `jte/checkbox.jte` | — (indeterminate + aria-invalid/error state now shipped) |
...
| Dialog | **full** | `wire/dialog` | — (focus-trap + Escape + Trigger/Header/Footer/Title/Description/Close slots now shipped) |
...
| Field | **full** | `jte/field` | — (FieldGroup/Set/Legend + orientation + Separator + auto aria-invalid + FieldContent slot now shipped) |
| Hover Card | partial | `jte/hover-card` | open/close delays, positioning props, controlled open |
| Input | **full** | `jte/input.jte` | — (file-specific affordance + built-in helper text now shipped) |
| Input Group | partial | `jte/input-group` | Textarea variant, spinner/loading addon, block-end alignment, Button sub-component |
| Input OTP | **full** | `jte/input-otp` + enhancer | — (Separator + explicit Group primitive + onComplete now shipped) |
...
| Native Select | **full** | `jte/native-select` | — (optgroup + size variants now shipped) |
| Navigation Menu | partial | `jte/navigation-menu` | viewport shared panel, animated indicator, asChild, keyboard open |
| Pagination | **full** | `jte/pagination.jte` | — (asChild router-Link now shipped) |
...
| Radio Group | **full** | `jte/radio-group` | — (aria-invalid/error state now shipped) |
| Resizable | **missing** | — | PanelGroup, Panel sizing/collapsible, ResizableHandle, persisted layout |
| Scroll Area | **full** | `jte/scroll-area` | — (custom scrollbar + ScrollBar component + type modes now shipped) |
...
| Sheet | **full** | `wire/sheet` | — (focus-trap + Escape + composable Trigger slot now shipped) |
| Skeleton | **full** | `jte/skeleton.jte` | — (pulse + shape + sizing) |
| Slider | **full** | `jte/slider` | — (range/multiple thumbs + controlled + vertical orientation now shipped) |
...
| Textarea | **full** | `jte/textarea.jte` | — (built-in helper text now shipped) |
...
| Form | **full** | `jte/form` + validation orchestration | — (server-side validation→message orchestration, Form/FormField/FormMessage set, auto aria-invalid/aria-describedby wiring now shipped) |
