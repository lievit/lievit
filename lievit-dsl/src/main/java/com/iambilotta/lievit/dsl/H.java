 ...
 * tree (ADR-0003, ADR-0015). Import statically and write the view inline:
 *
 * <pre>{@code
 * import static com.iambilotta.lievit.dsl.H.*;
 *
 * @LievitRender
 * Html view() {
 *     return div(
 ...
 * modules, which keeps the output CSP-safe by construction (ADR-0015), the same rule the JTE
 * templates obey.
 *
...
public final class H {
     ...
     * {@code @Wire String} carrying {@code <script>} is rendered inert (ADR-0015).
     *
     ...
     * (ADR-0015). Pass only server-provenance / already-sanitized markup.
     *
