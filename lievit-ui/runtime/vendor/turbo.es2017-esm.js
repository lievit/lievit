/*
 * VENDORED — Turbo Drive (first-party copy, do not edit by hand).
 *
 *   source:   @hotwired/turbo  https://github.com/hotwired/turbo
 *   dist:     dist/turbo.es2017-esm.js  (the package's published ESM build, unmodified below)
 *   version:  8.0.23
 *   license:  MIT (37signals LLC) — see the upstream banner directly below.
 *   vendored: 2026-06-23 by Francesco Bilotta.
 *
 * WHY VENDORED (ADR-0084 reframed clause-1 + ADR-0085): lievit's supply-chain posture is
 * first-party-only — no CDN, no runtime npm dependency that can be swapped under us. We pin a
 * reviewed dist build of a reputable, stable, low-supply-chain-risk library (37signals, MIT, no
 * install-time scripts) and check it in, exactly as a `lievit add` copy-in component would be. The
 * file is CSP-clean (no dynamic-code-eval primitives); verified at vendor time.
 *
 * WHAT IS USED: Turbo **Drive** only (it auto-`start()`s on import — see the `start()` call near the
 * end of this file). Frames (`<turbo-frame>`) and Streams (`<turbo-stream>`) are opt-in via markup
 * and lievit ships none, so their custom elements register but stay dormant. The single published
 * ESM bundle is the canonical 37signals artifact; we deliberately do NOT carve Drive out of it
 * (non-canonical surgery on a vendored dist is exactly the accidental complexity ADR-0084 forbids).
 *
 * UPGRADE: `npm pack @hotwired/turbo@<new>` → copy `dist/turbo.es2017-esm.js` over this body →
 * re-add this header → re-verify the body has no dynamic-code-eval primitive (scan the JS below this
 * banner; both counts must be 0) → run the lievit-ui vitest + tsc + build. Bump the version + date
 * above. Do not hand-edit otherwise.
 */
/*!
Turbo 8.0.23
